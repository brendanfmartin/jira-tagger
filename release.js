...
console.log(`git --git-dir ${pathtodir}/.git log ${tag1}..${tag2} > ${pathtooutput}.log`);

//
// if (!!pathtodir || !!tag1 ||!!tag2 || !!pathtooutput) {
//   return 0;
// }

exec(`git --git-dir ${pathtodir}/.git log ${tag1}..${tag2} > ${pathtooutput}.log`,
  (err, stdout, stderr) => {
    if (err) {
      console.log(`err: ${err}`);
    }
...
setTimeout(() => {
  logFileLines = fs.readFileSync(`${pathtooutput}.log`, 'ascii').toString().split("\n");
  console.log(`${pathtooutput}.log`);
  ...
  fs.writeFile(`${pathtooutput}-jira.txt`, newArr, (err) => {
    if (err) {
